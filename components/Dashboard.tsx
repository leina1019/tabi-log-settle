...
const Dashboard: React.FC<Props> = ({
  ...
  const formattedDateRange = useMemo(() => {
    if (!tripStartDate) return t('dashboard.dateNotSet');
    ...
    return e ? `${sS} - ${e.toLocaleDateString(undefined, { month: 'short', day: 'numeric' })}` : sS;
      ...
      >
        ...
        <div className="absolute bottom-4 left-6 right-6 text-white">
          ...
          <div className="flex justify-between items-end">
            <div>
              <h2 className="text-xl sm:text-2xl font-sans font-bold tracking-wide mb-0.5 leading-tight">{tripName || t('common.untitledTrip')}</h2>
            ...
            <div className="flex flex-col items-end gap-1">
              ...
              <span className="text-[8px] sm:text-[10px] font-medium opacity-60">{userProfiles.length}{t('common.person')} {t('dashboard.travelers')}</span>
            </div>
          </div>
        </div>
      </button>
            ...
            return (
              <button key={m.id} onClick={() => setSelectedMemberId(m.id)} className="w-full text-left glass p-5 rounded-2xl flex items-center justify-between active:scale-[0.98] transition-all hover:shadow-md group">
                ...
                <div className="text-right">
                  <p className="text-xl font-sans font-bold text-ink leading-none">{Math.round(m.paid).toLocaleString()}<span className="text-[10px] ml-1 font-sans font-normal opacity-60">{t('common.jpy')}</span></p>
                </div>
              </button>
            );
          })}
        ...
        <div className="grid grid-cols-2 gap-3">
          ...
          {summaryData.foreignCurrencies.length > 0 ? (
            <div className="glass p-5 rounded-3xl border-white/40 flex flex-col items-center justify-center text-center shadow-sm">
              ...
              <p className="text-[9px] font-bold text-ink-light mt-1.5">{summaryData.foreignCurrencies[0].currency} {t('dashboard.foreignTotal')}</p>
            </div>
          ) : (
            <div className="glass p-5 rounded-3xl border-white/40 flex flex-col items-center justify-center text-center shadow-sm">
        ...
        <div className="glass p-5 rounded-[32px] space-y-8">
          <div>
            <div className="flex justify-between items-center mb-4">
              <h4 className="text-[9px] font-bold text-ink uppercase tracking-wider flex items-center gap-1.5"><span className="w-1.5 h-3 bg-ocean-light rounded-full"></span>{t('dashboard.dailyUserExpense')}</h4>
              <p className="text-[8px] text-ink-light font-bold">{t('dashboard.dailyGraph')}</p>
            </div>
                              ...
                              {payload.map((entry: any, index: number) => (
                                <div key={index} className="flex items-center justify-between gap-4">
                                  ...
                                  <span className="text-[10px] font-sans font-black text-ink">
                                    {Math.round(Number(entry.value)).toLocaleString()}{t('common.jpy')}
                                  </span>
                                </div>
                              ))}
                            </div>
                            <div className="pt-2 border-t border-dashed border-surface-gray-mid flex justify-between items-center gap-4">
                              <span className="text-[9px] font-black text-primary uppercase tracking-widest">{t('common.total')}</span>
                              <span className="text-xs font-sans font-black text-primary">
                                {Math.round(total).toLocaleString()}{t('common.jpy')}
                              </span>
                            </div>
                          </div>
                        );
          ...
          <div className="border-t border-surface-gray-mid/50 pt-6">
            <div className="flex justify-between items-center mb-4">
              <h4 className="text-[9px] font-bold text-ink uppercase tracking-wider flex items-center gap-1.5"><span className="w-1.5 h-3 bg-premium-gold rounded-full"></span>{t('dashboard.payerRatio')}</h4>
              <p className="text-[8px] text-ink-light font-bold">{t('dashboard.memberExpense')}</p>
            </div>
            <div className="h-[140px] w-full">
              <ResponsiveContainer width="100%" height="100%">
                <BarChart data={memberStats} layout="vertical" margin={{ left: -10, right: 20 }}>
                  ...
                  <Tooltip cursor={{ fill: 'rgba(207,168,110,0.05)' }} contentStyle={{ background: 'rgba(255,255,255,0.95)', border: 'none', borderRadius: '12px', boxShadow: '0 10px 30px rgba(0,0,0,0.1)', fontSize: '10px' }} formatter={(v: number) => [`${Math.round(v).toLocaleString()}${t('common.jpy')}`, t('dashboard.paidAmount')]} />
      ...
      {isEditingBudget && (
        <div className="fixed inset-0 z-[110] flex items-center justify-center p-4 bg-primary/20 backdrop-blur-sm" onClick={() => setIsEditingBudget(false)}>
          <div className="bg-white w-full max-w-xs rounded-3xl p-6 shadow-2xl border border-surface-gray-mid" onClick={e => e.stopPropagation()}>
            <h3 className="text-lg font-bold text-ink mb-4">{t('dashboard.editBudget')}</h3>
            <div className="space-y-4">
              <div>
                <label className="block text-[10px] font-bold text-ink-sub mb-1 uppercase tracking-widest">{t('dashboard.targetBudget')}</label>
                <input
                  type="number"
        ...
        <div className="fixed inset-0 z-[110] flex items-center justify-center p-4 bg-ocean-dark/40 backdrop-blur-md" onClick={() => { setIsEditingTrip(false); setError(null); }}>
          <div className="bg-white w-full max-w-sm rounded-[32px] p-6 shadow-2xl border border-surface-gray-mid overflow-y-auto max-h-[90dvh]" onClick={e => e.stopPropagation()}>
            <div className="flex justify-between items-center mb-6">
              <h3 className="text-xl font-sans font-bold text-ink">{t('dashboard.tripSettings')}</h3>
            ...
            <div className="space-y-5">
              ...
              <div className="relative aspect-[21/9] rounded-2xl overflow-hidden shadow-md group cursor-pointer" onClick={() => fileInputRef.current?.click()}>
                ...
                <div className="absolute inset-0 bg-black/30 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity">
                  <span className="bg-white/20 backdrop-blur-md border border-white/40 text-white px-4 py-2 rounded-full text-[10px] font-bold uppercase tracking-widest">{t('dashboard.editCoverImage')}</span>
                </div>
                <input type="file" ref={fileInputRef} className="hidden" accept="image/*" onChange={handleImageChange} />
              </div>

              <div>
                <label className="block text-[10px] font-bold text-ink-sub mb-1 uppercase tracking-widest">{t('dashboard.tripDestination')}</label>
                <input
                  type="text"
                  placeholder={t('dashboard.tripDestPlaceholder')}
                  className="w-full bg-surface-gray border border-surface-gray-mid rounded-xl p-3 text-sm text-ink outline-none focus:border-primary"
                  value={tempTripName}
          ...
          <div className="bg-white w-full max-w-md h-[92dvh] sm:h-auto sm:max-h-[85dvh] sm:rounded-[40px] rounded-t-[40px] shadow-2xl overflow-y-auto border-t border-surface-gray-mid flex flex-col relative" onClick={e => e.stopPropagation()}>
            <div className="sticky top-0 z-20 bg-white/80 backdrop-blur-md px-6 py-5 border-b border-surface-gray-mid flex justify-between items-start">
              <div className="flex items-center gap-4">
                ...
                <div>
                  <h3 className="text-xl sm:text-2xl font-sans font-black text-ink leading-tight">{selectedMemberId === 'ALL' ? t('dashboard.allMembersSummary') : getProfile(selectedMemberId).displayName}</h3>
            ...
            <div className="p-6 space-y-8">
              <div className="relative p-5 rounded-2xl bg-white border border-surface-gray-mid shadow-sm space-y-4">
                {selectedMemberId === 'ALL' ? (
                  <>
                    <div className="flex justify-between items-end">
                      <div><p className="text-[9px] font-bold text-ink-sub uppercase tracking-widest mb-1">{t('dashboard.totalExpense')}</p><h4 className="text-2xl font-sans font-black text-ink">{Math.round(totalJPY).toLocaleString()}<span className="text-xs font-bold ml-1 opacity-50">{t('common.jpy')}</span></h4></div>
                      <div className="text-right"><p className="text-[9px] font-bold text-ink-sub uppercase tracking-widest mb-1">{t('dashboard.budgetUsedRate')}</p><span className={`text-xs font-black ${budgetPercentage > 100 ? 'text-rose-500' : 'text-ocean-dark'}`}>{budgetPercentage.toFixed(1)}%</span></div>
                    </div>
                  ...
                  <div className="space-y-4">
                    <div className="grid grid-cols-2 gap-4">
                      <div className="space-y-0.5"><p className="text-[8px] font-bold text-ink-sub uppercase tracking-widest">{t('dashboard.actualPaid')}</p><p className="text-lg font-sans font-black text-ink">{Math.round(detailData.stats!.paid).toLocaleString()}</p></div>
                      <div className="space-y-0.5 text-right"><p className="text-[8px] font-bold text-ink-sub uppercase tracking-widest">{t('dashboard.shouldBurden')}</p><p className="text-lg font-sans font-bold text-ink-sub">- {Math.round(detailData.stats!.cost).toLocaleString()}</p></div>
                    </div>
                    <div className="pt-3 border-t border-surface-gray-mid flex justify-between items-center text-xs font-bold uppercase tracking-widest">{t('dashboard.balance')}<span className={`text-xl font-black ${detailData.stats!.balance >= 0 ? 'text-emerald-500' : 'text-rose-500'}`}>{detailData.stats!.balance >= 0 ? '+' : ''}{Math.round(detailData.stats!.balance).toLocaleString()}</span></div>
                  </div>
                )}
              </div>
              <div className="space-y-10">
                <section>
                  <div className="flex items-center gap-2 mb-4"><span className="w-1.5 h-4 bg-premium-gold rounded-full"></span><h4 className="text-[10px] font-bold text-ink uppercase tracking-[0.3em]">{t('dashboard.expenseCategory')}</h4></div>
                  <div className="h-[180px] w-full"><ResponsiveContainer><PieChart><Pie data={detailData.categoryData} cx="50%" cy="50%" innerRadius={50} outerRadius={70} paddingAngle={5} dataKey="value" stroke="none">{detailData.categoryData.map((e, i) => <Cell key={i} fill={COLORS[i % COLORS.length]} />)}</Pie><Tooltip contentStyle={{ background: 'rgba(255,255,255,0.95)', border: 'none', borderRadius: '12px', boxShadow: '0 10px 30px rgba(0,0,0,0.1)', fontSize: '10px' }} formatter={(v: number) => [`${Math.round(v).toLocaleString()}${t('common.jpy')}`, t('dashboard.expenseAmount')]} /></PieChart></ResponsiveContainer></div>
                </section>
                <section className="space-y-4">
                  {selectedMemberId === 'ALL' ? (
                    ...
                    <>
                      ...
                      {detailData.iPaidForOthers.length === 0 && detailData.othersPaidForMe.length === 0 && (
                        <div className="py-6 text-center">
                          ...
                          <p className="text-xs font-bold text-ink-light">{t('dashboard.noExpenseRecord')}</p>
                        </div>
                      )}
                      {detailData.iPaidForOthers.map(i => (
                        <div key={i.id} className="flex justify-between items-center p-3.5 rounded-xl bg-emerald-50 border border-emerald-100"><span className="text-xs font-bold">{getProfile(i.id).displayName}{t('dashboard.paidForOthers')}</span><span className="text-xs font-black text-emerald-600">+{Math.round(i.value).toLocaleString()}</span></div>
                      ))}
                      {detailData.othersPaidForMe.map(o => (
                        <div key={o.id} className="flex justify-between items-center p-3.5 rounded-xl bg-rose-50 border border-rose-100"><span className="text-xs font-bold">{getProfile(o.id).displayName}{t('dashboard.othersPaidForMe')}</span><span className="text-xs font-black text-rose-500">-{Math.round(o.value).toLocaleString()}</span></div>
                      ))}
                    </>
                  )}
                </section>
